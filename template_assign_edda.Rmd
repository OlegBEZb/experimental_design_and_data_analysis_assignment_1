...
\$\\mu\$. Evaluate the sample size needed to provide that the length of
the 97%-CI is at most 2. Compute a bootstrap 97%-CI for \$\\mu\$ and
compare it to the above CI.
...
t-confidence interval of level 3% for $\mu$:

```{r}
CI_97 <- list(mu - ta*s/sqrt(n), mu + ta*s/sqrt(n))
CI_97
```
